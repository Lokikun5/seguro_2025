    ...
    <main class="admin-content">
        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="meta_title" class="form-control" value="{{ old('meta_title', $event->meta_title) }}" required>
            </div>

            <div class="mb-3">
                <label for="meta_description">Meta Description</label>
                <input type="text" name="meta_description" class="form-control" value="{{ old('meta_description', $event->meta_description) }}" required>
            </div>
            ...
            <div class="mb-3">
                <label for="start_date">Date de l’événement</label>
                <input type="date" name="start_date" class="form-control" value="{{ old('start_date', $event->getRawOriginal('start_date')) }}" required>
            </div>

            <div class="mb-3">
                <label for="performance">Performance</label>
                <input type="text" name="performance" class="form-control" value="{{ old('performance', $event->performance) }}" required>
            </div>

            <div class="mb-3">
                <label for="introduce">Introduction</label>
                <input type="text" name="introduce" class="form-control" value="{{ old('introduce', $event->introduce) }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                @if($event->profile_pic)
                    <div class="mb-2">
                        <img src="{{ $event->profile_pic }}" alt="{{ $event->title }}" width="100">
                    </div>
                @endif
            ...
            <div class="form-check mb-4">
                ...
                <input class="form-check-input" type="checkbox" name="active" value="1" id="active" {{ old('active', $event->active) ? 'checked' : '' }}>
        ...
        {{-- Galerie photo --}}
        @if($event->media->where('type', 'photo')->isNotEmpty())
            <hr>
            <h4 class="mt-4">Photos existantes</h4>
            <div class="d-flex flex-wrap gap-3">
                @foreach($event->media->where('type', 'photo') as $photo)
        ...
        @if($event->media->where('type', 'video')->isNotEmpty())
            ...
            <div class="d-flex flex-wrap gap-3">
                @foreach($event->media->where('type', 'video') as $video)
                    <div class="text-center">
